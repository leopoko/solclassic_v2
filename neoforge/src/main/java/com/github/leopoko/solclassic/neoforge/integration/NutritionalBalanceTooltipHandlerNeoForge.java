...
/**
 * Nutritional Balance MODツールチップ連携（NeoForge版・クライアント専用）。
 ...
 * - 通常食べ物: 品質値に減衰倍率を適用
 */
public class NutritionalBalanceTooltipHandlerNeoForge {

    public static void register() {
        NeoForge.EVENT_BUS.register(NutritionalBalanceTooltipHandlerNeoForge.class);
    }
    ...
    public static void onItemTooltip(ItemTooltipEvent event) {
        ...
        replaceNBLine(event.getToolTip(), targetItem, targetStack, player.level(), multiplier);
    }

    /**
     * NB APIから栄養素・品質値を取得し、減衰適用済みのツールチップ行に差し替える。
     */
    private static void replaceNBLine(List<Component> tooltips, Item item, ItemStack itemStack, Level level, float multiplier) {
