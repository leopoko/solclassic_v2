...
## 1.21.1 移行担当AI向けの注意事項

このセクションは、1.21.1ブランチへの移行作業を行うAI向けの情報です。mainブランチで修正されたバグのうち、1.21.1ブランチにも影響する可能性があるものを記録しています。移行作業時にはこれらの修正が1.21.1側にも適切に反映されていることを確認してください。

### FoodChestMenu スロット差し替え時の `slot.index` 未設定バグ（修正済み）

- **対象ファイル**: `common/src/main/java/com/github/leopoko/solclassic/container/FoodChestMenu.java`
- **症状**: バスケットに4つ目のアイテムを入れると、1番目のスロット（スロット0）のアイテムが上書きされて消失する
- **原因**: `FoodChestMenu` のコンストラクタで `this.slots.set(i, newSlot)` によりスロットを差し替えているが、`addSlot()` 経由ではないため新しいスロットの `Slot.index` フィールド（メニュー内でのスロット位置ID）がデフォルトの `0` のままになっていた。全ての差し替えスロットが `index = 0` を持つことで、スロット識別が正しく行われず、特に Mohist のような Bukkit ブリッジ環境でアイテムの上書きが発生していた
- **修正内容**: 差し替えた各スロットに `slot.index = 正しいメニュースロット位置` を明示的に設定
- **1.21.1 での注意**: 1.21.1 では `Slot` クラスのフィールド構成やアクセス修飾子が変更されている可能性がある。`Slot.index` が存在するか、同等のフィールド/メソッドがあるかを確認すること。また、1.21.1 でコンテナメニューの仕組みが変わっている場合は、そもそも `this.slots.set()` によるスロット差し替えパターン自体を見直し、`addSlot()` を正しく利用する方式への変更を検討すること
